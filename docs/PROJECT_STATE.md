...
Обновлено: 2026-08-29 (вехи: аккаунт МойСклад; Шаг 6c в НОВОЙ редакции — панель ручного
заказа у поставщика; Шаг 7 — оплата ЮKassa, тестовый контур; 7b — живая проверка наличия,
код выдан, тест не подтверждён — см. п.7.0)
...
| 7b | Живая проверка наличия у поставщика перед оплатой (fail-open, 6 с) + свежая закупка в снимок маржи | 🔶 код выдан 2026-08-29, тест не подтверждён |
...
| 8 | Доставка: зоны Москва/МО, тарифы (поля адреса в Order уже структурные) | ⬜ СЛЕДУЮЩИЙ крупный |
...
- ПОСТАВЩИК — АБСОЛЮТ ТРЕЙД, eCommerce API v3. Токен в .env (SUPPLIER_API_TOKEN, 1 год).
  ...
  не отправлено): лимиты частоты; цены с НДС или без; битые фото; категории с code=null.
  Вопрос про тестовый режим API-заказов ИСКЛЮЧЁН — заказы делаем вручную (6c).
...
- src/app/checkout/actions.ts — saveCheckout (шаг 1 → cookie lm_checkout, сутки),
  placeOrder (шаги 7/7b): проверка по БД → ЖИВАЯ проверка остатков у поставщика
  (AvailabilityAndPrice, таймаут 6 с, fail-open → работаем по БД; свежая закупка пишется
  в снимок basePrice) → транзакция Order + OrderItem → платёж ЮKassa → redirect на
...
0) ДОПРОВЕРИТЬ 7b (код выдан 2026-08-29, применён ли и протестирован — уточнить у
   владельца): обычный заказ через чекаут проходит, в терминале dev-сервера НЕТ строки
   «[checkout] живая проверка у поставщика недоступна». Потом закоммитить всё
   (7b + package.json с cross-env + этот файл), если ещё не.
1) Следующий крупный шаг — 8 (доставка: зоны Москва/МО, тарифы, deliveryCost в заказ
   и в чек ЮKassa).
2) Хвосты без кода (в любой момент): отправить письмо api@absoluttrade.ru (текст готов);
   внести реквизиты ИП в МойСклад; выбрать SMS-провайдера (src/lib/sms — один файл);
