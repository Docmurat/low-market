...
Обновлено: 2026-08-29 (вехи: 7b; Шаг 8 — доставка; 8b — адресные подсказки DaData,
зона по официальному региону, блок курьера вне Москвы/МО)
...
0) Убедиться, что закоммичено: шаг 8, задача 8b (api/address-suggest, AddressSuggest,
   CheckoutForm, delivery/calc, checkout-shared, actions, confirm) и этот файл.
1) Следующий крупный шаг по карте — 9 (отзывы + бонус за отзыв + модерация в админке).
   Обсудить с владельцем перед кодом: механика бонуса (что даём и как учитываем).
